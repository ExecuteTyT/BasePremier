...
- Location: tests/e2e/live-qa.spec.ts:92:9
...
Call log:
  ...
  - waiting for locator('a[href^="tel:"]').first()
    11 × locator resolved to <a href="tel:+79179183877" class="hidden font-mono text-body-sm text-fg-muted transition-colors duration-base hover:text-fg-primary lg:block">+7 (917) 918-38-77</a>
...
```ts
  ...
  57  |         console.warn(`  H1 [${vp.name}px] ${pg.route}: "${h1Text.trim()}"`);
  58  |
  ...
  68  |           console.warn(`  CTA visible [${vp.name}px] ${pg.route}: ${isVisible}`);
  69  |           expect(isVisible, `Primary CTA visible on ${pg.route} @ ${vp.name}px`).toBeTruthy();
  70  |         } else {
  71  |           console.warn(`  CTA not found [${vp.name}px] ${pg.route}`);
  72  |         }
  73  |
  74  |         // 3. Console errors
  ...
  91  |       console.warn(`  tel: link [${vp.name}px]: ${href}`);
  92  |     });
  93  |
...
> 97  |       await page.waitForLoadState("networkidle").catch(() => {});
  ...
  104 |       console.warn(`  Cookie banner visible [${vp.name}px]: ${bannerVisible}`);
  105 |
  ...
  120 |           console.warn(`  Hero CTA in viewport [${vp.name}px]: ${inViewport}, y=${Math.round(box.y)}`);
  121 |         }
  122 |       }
  123 |     });
  124 |   });
