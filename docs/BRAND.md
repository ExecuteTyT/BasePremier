...
**«Concrete & Midnight»** — бруталистский интерьер салона, пойманный в digital. Бетонные текстуры, ночное небо за окном, единственная лампа подсветки, никакой лишний шум. Зритель должен почувствовать: *«здесь думают, прежде чем что-то сделать. И делают это очень дорого».*
...
<!-- Section H2 -->
<h2 class="font-display text-display-md leading-tight tracking-tight">
  О нас
</h2>
...
<!-- Overline / label -->
<span class="font-sans text-overline tracking-overline uppercase text-fg-muted">
  Услуга
</span>
```
...
.font-display.text-body, .font-display.text-caption {
...
@media (min-width: 1280px) { .container { padding-inline: var(--gutter-desktop); } }
```
...
@media (max-width: 639px) {
  .grid-12 { grid-template-columns: repeat(4, 1fr); gap: 1rem; }
}
```
...
```css
@theme {
  --ease-out-expo:     cubic-bezier(0.19, 1.0, 0.22, 1.0);   /* основной */
...
export function playSound(name: string) {
  ...
  sounds[name]!.play().catch(() => {/* autoplay blocked */});
}
...
*При любом сомнении в реализации — открой dev-страницу `/_dev/tokens` (см. ROADMAP, тикет BR-09), там визуально все токены применены к компонентам. Если что-то выглядит «странно» — это сигнал, что токен в коде разошёлся с этим документом. Тогда либо обновляем код, либо обновляем документ — и фиксируем git-коммитом.*
